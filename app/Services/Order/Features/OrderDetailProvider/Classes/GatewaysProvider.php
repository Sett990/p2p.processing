...
class GatewaysProvider
{
    ...
    /**
     * Генерирует уникальный ключ кэша на основе параметров конструктора
     */
    protected function makeCacheKey(): string
    {
        $merchantId = $this->merchant->id;
        $amount = $this->amount->toBeauty();
        $currency = $this->currency?->getCode() ?? 'null';
        $gateway = $this->gateway?->code ?? 'null';
        return 'gateways_provider_' . md5(implode('|', [$merchantId, $amount, $currency, $gateway]));
    }
}

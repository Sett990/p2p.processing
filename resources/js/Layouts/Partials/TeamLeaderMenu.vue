...
<template>
    <ul class="menu w-full">
        ...
        <li :class="[{ 'bg-base-content/10 rounded-lg': route().current('leader.promo-codes.*') }]">
            <span
                @click="router.visit(route('leader.promo-codes.index'), { preserveScroll: true })"
                @keydown.enter.space="router.visit(route('leader.promo-codes.index'), { preserveScroll: true })"
                role="link"
                tabindex="0"
            >
                <svg class="size-5 opacity-30" stroke-width="1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">

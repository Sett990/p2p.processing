...
const { paymentGatewayCreateModal } = storeToRefs(modalStore);
...
const submit = () => {
    ...
    errors.value = {};
    axios.post(route('admin.payment-gateways.store'), toFormData(), {
...
watch(
    () => paymentGatewayCreateModal.value.showed,
    (state) => {
        if (state) {
            resetForm();
            loadData();
...
<template>
    <Modal :show="paymentGatewayCreateModal.showed" @close="close" maxWidth="4xl">
        <ModalHeader @close="close" title="Создание платежного метода" />
